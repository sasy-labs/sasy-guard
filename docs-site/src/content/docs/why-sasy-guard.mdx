...
Fair question, and the honest answer starts with a concession: **for the
simplest checks, a hook really is all you need.** Blocking one command by
pattern is a few lines of shell — if that's your whole threat model, roll your
own.
...
<Aside type="note" title="Honest scope (so we're not overselling)">
...
<CardGrid>
  <Card title="🔨 Your own hook">
    `grep 'curl.*| *sh'`. Misses `wget | bash`, `curl -o /tmp/x && sh /tmp/x`,
    and fetch-then-exec split across calls — and can't tell a trusted host from
    an unknown one. You also wanted *ask*, not *deny*, so now you're hand-rolling
    the verdict protocol and the reason string.
  </Card>
  <Card title="🛡️ sasy-guard">
    Host-aware (unknown source), normalized across pipe/temp-file variants, and
    returns **ask** with a `[SASY] …` reason the agent relays back to you.
  </Card>
</CardGrid>

## The guard the agent can't switch off

The two rules that protect enforcement *itself* are the ones a hook
structurally cannot deliver — because the hook is co-located with the thing it's
supposed to guard.
...
<CardGrid>
  ...
  <Card title="🛡️ sasy-guard">
    Enforcement runs in a separate **locked engine** plus a native binary under
    `~/.sasy`. Writing agent/editor config (hooks, MCP servers) is a first-class
    **deny**, and removing the project setting doesn't remove the guard.
  </Card>
</CardGrid>
...
<CardGrid>
  ...
  <Card title="🛡️ sasy-guard">
    The policy is baked into a locked engine the session can't reconfigure or
    disable, so the endpoint hijack **and** the config-tampering that would defeat
    it are both blocked.
  </Card>
</CardGrid>
...
<CardGrid>
  <Card title="🔨 Your own hook">
    "Block `git push` unless gitleaks ran." But did it run *clean*, and did the
    scan *cover the edits made this session*? Answering that means actually
    running gitleaks and correlating it with the files touched — not grepping the
    transcript for the word "gitleaks".
  </Card>
  <Card title="🛡️ sasy-guard">
    The daemon runs `gitleaks` out-of-band and gates the push on a clean scan
    that **covers the session's edits**.
  </Card>
</CardGrid>
